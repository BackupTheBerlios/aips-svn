//
// C++ Implementation: cdisplaywindow
//
// Description: 
//
//
...
#ifdef USE_RENDERING
CDisplayWindow::CDisplayWindow() : QWidget( NULL ), theActualImage(NULL), theActualOverlay(NULL)
{
  dImageLut[0] = 0.0;
  dImageLut[1] = 127.0;
  dOverlayLut[0] = 0.0;
  ...
  theOverlay = vtkImageActor::New();
  theOverlayColorsMapper = vtkImageMapToColors::New();
  theOverlayColorsMapper->SetLookupTable( theOverlayLookupTable );
  theOverlayColorsMapper->SetOutputFormatToRGBA();
  theOverlay->SetInput( theOverlayColorsMapper->GetOutput() );
  theImageBlender = vtkImageBlend::New();
  theImageBlender->SetInput( 0, theImageColorsMapper->GetOutput() );
  theImageBlender->SetInput( 1, theOverlayColorsMapper->GetOutput() );
  theImage->SetInput( theImageBlender->GetOutput() );
...
void CDisplayWindow::loadImageLutFile()
{
  string path;
...
void CDisplayWindow::loadOverlayLutFile()
{
  string path;
  if ( getGlobalConfiguration().isDefined( "AIPS_LUT" ) )
    path = getGlobalConfiguration().getString( "AIPS_LUT" );
...
void CDisplayWindow::overlayMaxDataChanged()
{
  theOverlayDataMax->setValue( static_cast<int>( theOverlayMaxDisplay->text().toDouble()
    / ( dOverlayClampValues[1] - dOverlayClampValues[0] ) * 1000.0 ) );
}

void CDisplayWindow::toggleImageTransparency( int i )
{
  if ( i == QButton::On )
  {
    theImageLookupTable->SetTableValue( 0, 0.0, 0.0, 0.0, 0.0 );
...
void CDisplayWindow::toggleOverlayTransparency( int i )
{
  if ( i == QButton::On )
  {
    theOverlayLookupTable->SetTableValue( 0, 0.0, 0.0, 0.0, 0.0 );
...
void CDisplayWindow::toggleImageInterpolation( int i )
{
  if ( i == QButton::On )
  {
...
void CDisplayWindow::toggleOverlayInterpolation( int i )
{
  if ( i == QButton::On )
  {
    theOverlay->InterpolateOn();
  }
  else
  {
    theOverlay->InterpolateOff();
  }
  if ( !doNotUpdate ) interactor->Render();
}

void CDisplayWindow::loadLookupTable( std::string sFilename, bool bForImage )
{
  if ( bForImage )
...
void CDisplayWindow::setImageUpperClamp( double dValue )
{
...
void CDisplayWindow::setOverlayUpperClamp( double dValue )
{
...
void CDisplayWindow::setImageLowerClamp( double dValue )
{
...
void CDisplayWindow::setOverlayLowerClamp( double dValue )
{
...
void CDisplayWindow::updateImageMax( int i )
{
...
void CDisplayWindow::updateOverlayMax( int i )
{
...
void CDisplayWindow::updateImageMin( int i ) 
{
  setImageLowerClamp( static_cast<double>( i ) / 1000.0 * ( dImageLut[1] - dImageLut[0] ) );
...
void CDisplayWindow::updateOverlayMin( int i )
{
  setOverlayLowerClamp( static_cast<double>( i ) / 1000.0 * ( dOverlayLut[1] - dOverlayLut[0] ) );
...
void CDisplayWindow::testImageDataRange( double minRange, double maxRange )
{
FBEGIN;
  doNotUpdate = true;
...
void CDisplayWindow::testOverlayDataRange( double minRange, double maxRange )
{
FBEGIN;
  doNotUpdate = true;
...
void CDisplayWindow::setOverlay( vtkImageData* anOverlay )
{
 FBEGIN;
  doNotUpdate = true;
  theOverlayLookupTable->SetTableRange( dOverlayClampValues );
